						...
						if xscene ~= nil then
							...
							if records > 0 then
								...
								for rr = 1, records do
									...
									local pritab = {};
									local bunktb = {};
									local polnum = table.getn(xscene[2][rr][2]);
									local polidx = 1;
									while polidx <= polnum do
										local idxtab = {};
										local tmppri = {};
										local tbunks = {};
										for k, v in pairs(xscene[2][rr][2][polidx]) do
											if k > 0 then
												if type(v) == "table" then
													if v[0] ~= "FlightBaseInfos" and v[0] ~= "PriceInfos" and v[0] ~= "NoSalesStr" then
														idxtab[v[0]] = v[1];
									...
									for i = 1, 2 do
										local tmpfid = "";
										for j = 1, table.getn(xscene[2][rr][1][i][3]) do
											-- ngx.say(type(xscene[1][i][3][j]))
											local tmpseg = {};
											local fltkey = {};
											for k, v in pairs(xscene[2][rr][1][i][3][j]) do
												if k > 0 then
													if type(v) == "table" then
														if v[0] ~= "Stops" then
															tmpseg[v[0]] = v[1];
								...
								if table.getn(bigtab) > 0 then
									...
									print(code)
									print("-----RT caculation is NULL of {" .. r .. "}-------")
									print(status)
									print(body)
								end
								print("+++++++++++++++++++++++++++++++++++++++++++++++")
